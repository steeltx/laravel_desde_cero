    ...
    <form method="POST" action="{{ route('products.update',['product' => $product->id]) }}">
        ...
        <div class="form-row">
            ...
            <select name="status" class="custom-select" required>
                <option {{ $product->status == 'available' ? 'selected' : '' }} value="available">Disponible</option>
                <option {{ $product->status == 'unavailable' ? 'selected' : '' }} value="unavailable">No Disponible</option>
            </select>
        </div>
        <div class="form-row">
